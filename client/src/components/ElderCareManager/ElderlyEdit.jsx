...
const ElderlyEdit = () => {
  ...
  useEffect(() => {
    const fetchElderData = async () => {
      try {
        const response = await apiRequest.get(`/elders/getSingleElder/${id}`);
        if (response.status) {
          // Ensure all arrays are initialized
          const data = response.data.elder;
          setFormData({
            ...data,
            ...
            previousHospitalizations: data.previousHospitalizations || [],
        ...
        toast.error(
          error?.response?.data?.message || 'Failed to load elder data'
        );
  ...
  const addListItem = (section) => {
    setFormData((prev) => ({
      ...prev,
      [section]: [...prev[section], ''], // Add an empty string as the default value
    }));
  };
  ...
  const removeInsuranceDoc = (index) => {
    setFormData((prev) => ({
      ...prev,
      insuranceInfo: {
        ...prev.insuranceInfo,
        documents: prev.insuranceInfo.documents.filter((_, i) => i !== index),
      },
    }));
  };
          ...
          <div>
            ...
            {formData.insuranceInfo.documents.map((doc, index) => (
              ...
              >
                ...
                <button
                  onClick={() => removeInsuranceDoc(index)}
        ...
        <CardContent>
          <div className="space-y-4">
            <UploadWidget
              ...
              onUploadSuccess={handleMedicalUpload}
              setLoading={setLoading}
            />
            {formData.medicalDocuments.map((doc, index) => (
              ...
              >
                ...
                <button
                  onClick={() => removeMedicalDoc(index)}
                  className="text-red-500 hover:text-red-700 ml-2"
                >
                  <Trash2 size={16} />
                </button>
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await apiRequest.patch(
        `/elders/updateElderData/${id}`,
        formData
      );

      if (response.status === 200) {
        toast.success('Profile updated successfully!');
        navigate(`/elderProfile/${id}`);
      }
    } catch (error) {
    ...
    <div className="max-w-4xl mx-auto p-6 mt-14">
      <Card className="relative">
        <CardHeader>
          ...
          <CardDescription>
            Update the resident{`'`}s information. Changes are saved
            immediately.
          </CardDescription>
        </CardHeader>
